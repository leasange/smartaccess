	...
	public partial class IMS_VEHICLE_RECORD
	{
		...
		public decimal Add(Maticsoft.Model.IMS_VEHICLE_RECORD model)
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@AccessChannel", SqlDbType.VarChar,50),
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update IMS_VEHICLE_RECORD set ");
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.IMS_VEHICLE_RECORD DataRowToModel(DataRow row)
		{
			Maticsoft.Model.IMS_VEHICLE_RECORD model=new Maticsoft.Model.IMS_VEHICLE_RECORD();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,PlateNo,Name,Depart,AccessChannel,ThroughForward,ThroughTime,ThroughResult,CapturePic ");
			strSql.Append(" FROM IMS_VEHICLE_RECORD ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from IMS_VEHICLE_RECORD T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "IMS_VEHICLE_RECORD";
